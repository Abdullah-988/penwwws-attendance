...
def show_groups_page(root, app_state):
    ...
    tk.Label(header, text="Choose Group", font=("Arial", 16)).pack(side="top", pady=5)
    ...
    try:
        res = requests.get(f"{API_BASE_URL}/device/school/group", headers={
        ...
        if res.status_code == 200:
            ...
            for group in groups:
                ...
                btn = tk.Button(scrollable_frame, text=name, width=40, height=2, anchor="w",
                                command=lambda g=group: open_groups_page(root, app_state, g))
                btn.pack(expand=True, pady=3)
    except Exception as e:
        print("Error fetching groups:", e)

def open_groups_page(root, app_state, group):
    app_state["selected_group"] = group
    show_groups_students_page(root, app_state)
