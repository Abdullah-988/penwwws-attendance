...
def show_session_students_page(root, app_state):
    ...
    tk.Button(header, text="← Back", command=lambda: show_subjects_presets_page(root, app_state)).pack(side="left")
    ...
    try:
        res = requests.get(f"{API_BASE_URL}/device/school/student/session/{app_state['selected_session_id']}", headers={
