...
def show_subjects_page(root, app_state):
    ...
    tk.Button(header, text="← Back", command=lambda: show_home_page(root, app_state)).pack(side="left")
    tk.Label(header, text="Choose Subject", font=("Arial", 16)).pack(side="top", pady=5)
    ...
    try:
        res = requests.get(f"{API_BASE_URL}/device/school/subject", headers={
...
def open_presets_page(root, app_state, subject):
    app_state["selected_subject"] = subject
    show_subjects_presets_page(root, app_state)
